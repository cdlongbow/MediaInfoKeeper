...
namespace MediaInfoKeeper.Store {
    public class ChaptersStore {
        ...
        public List<ChapterInfo> ReadFromFile(BaseItem item) {
            ...
            logger.Debug($"ChaptersStore 从文件读取章节信息: {item.FileName ?? item.Path}");
            return chapters;
        }

        public bool HasInFile(BaseItem item) {
            var hasInFile = ReadFromFile(item).Count > 0;
            logger.Debug($"ChaptersStore 检查文件是否包含章节信息: {item.FileName ?? item.Path} 结果={hasInFile}");
            return hasInFile;
        }
        ...
        public bool WriteToFile(BaseItem item) {
            ...
            if (document.Chapters != null && document.Chapters.Count > 0) {
                logger.Debug($"ChaptersStore Json写入章节信息跳过: {item.FileName ?? item.Path}");
                return false;
            }

            document.Chapters = CreateForPersist(item);
            SaveDocuments(documents, document, mediaInfoJsonPath);
            logger.Debug($"ChaptersStore 写入章节信息成功: {item.FileName ?? item.Path}");
            return true;
        }
        ...
        private List<ChapterInfo> CreateForPersist(BaseItem item) {
            ...
            foreach (var chapter in chapters) chapter.ImageTag = null;
        ...
        public MediaInfoDocument.MediaInfoRestoreResult ApplyToItem(BaseItem item) {
            if (item == null) {
                logger.Debug("ChaptersStore 恢复章节失败: 条目为空");
                return MediaInfoDocument.MediaInfoRestoreResult.Failed;
            }

            var existingChapters = itemRepository.GetChapters(item) ?? new List<ChapterInfo>();
            if (existingChapters.Count > 0) {
                logger.Debug($"ChaptersStore 恢复章节跳过: {item.FileName ?? item.Path} 已存在章节信息");
                return MediaInfoDocument.MediaInfoRestoreResult.AlreadyExists;
            }

            var chapters = ReadFromFile(item);
            if (chapters.Count == 0) {
                logger.Debug($"ChaptersStore 恢复章节失败: {item.FileName ?? item.Path} JSON 中无章节数据");
                return MediaInfoDocument.MediaInfoRestoreResult.Failed;
            }
            ...
            try {
                ...
                logger.Debug($"ChaptersStore 恢复章节到条目完成: {item.FileName ?? item.Path}");
                return MediaInfoDocument.MediaInfoRestoreResult.Restored;
            }
            catch (Exception e) {
                logger.Error($"ChaptersStore 恢复章节失败: {item.FileName ?? item.Path}");
        ...
        public bool DeleteFromFile(BaseItem item) {
            ...
            if (document?.Chapters == null || document.Chapters.Count == 0) {
                logger.Debug($"ChaptersStore 删除Json章节信息跳过: {item.FileName ?? item.Path}");
                return false;
            }
            ...
            if (!document.HasPayload()) {
                ...
                logger.Info($"ChaptersStore 删除Json章节信息成功并删除文件: {item.FileName ?? item.Path}");
                return true;
            }

            jsonSerializer.SerializeToFile(documents, mediaInfoJsonPath);
            logger.Info($"ChaptersStore 删除Json章节信息成功: {item.FileName ?? item.Path}");
            return true;
        }
