...
namespace MediaInfoKeeper.Store {
    public class MediaInfoDocument {
        ...
        private static readonly Regex TemplateTokenRegex = new(@"\{([A-Za-z0-9_]+)\}", RegexOptions.Compiled);
        ...
        private static string BuildSidecarPath(BaseItem item, string extension) {
            ...
            if (string.IsNullOrWhiteSpace(jsonRootFolder)) return Path.Combine(item.ContainingFolderPath, fileName);

            if (ContainsTemplateToken(jsonRootFolder)) return BuildTemplatePath(item, jsonRootFolder, fileName);
        ...
        private static string GetConfiguredRootFolder(BaseItem item, string jsonRootFolder) {
            if (item is Audio) return Path.Combine(jsonRootFolder, "music");
        ...
        private static string BuildTemplatePath(BaseItem item, string template, string fileName) {
            ...
            if (string.IsNullOrWhiteSpace(expandedPath)) return Path.Combine(item.ContainingFolderPath, fileName);
        ...
        private static string NormalizeTemplateValueBoundary(string template, Match match, string value) {
            ...
            if (IsDirectorySeparator(previousChar)) value = TrimPathRoot(value);
        ...
        private static string TrimWindowsPathRoot(string path) {
            if (path.Length >= 2 && char.IsLetter(path[0]) && path[1] == ':') return path.Substring(2);

            if (path.StartsWith(@"\\", StringComparison.Ordinal) || path.StartsWith("//", StringComparison.Ordinal)) {
                var separatorCount = 0;
                for (var i = 2; i < path.Length; i++) {
                    if (!IsDirectorySeparator(path[i])) continue;

                    separatorCount++;
                    if (separatorCount == 2) return path.Substring(i + 1);
                }
            }
        ...
        private static void DeleteSidecar(BaseItem item, IDirectoryService directoryService, string path, string label,
            string source) {
