...
namespace MediaInfoKeeper.Patch {
    ...
    public static class PatchMethodResolver {
        ...
        public static ConstructorInfo ResolveConstructor(
            ...
            var constructor = type.GetConstructor(exactProfile.BindingFlags, null, exactProfile.ParameterTypes, null);
        ...
        private static MethodInfo TryResolve(
            ...
            ILogger logger,
            string context) {
            if (type == null || !IsValidProfile(profile)) return null;
        ...
        private static MethodInfo ResolveSingle(Type type, MethodSignatureProfile profile) {
            ...
            var method = type.GetMethod(profile.MethodName, profile.BindingFlags, null, profile.ParameterTypes, null);
        ...
        private static bool IsMatch(MethodInfo method, MethodSignatureProfile profile) {
            if (method == null || profile == null) return false;

            if (profile.IsStatic.HasValue && method.IsStatic != profile.IsStatic.Value) return false;

            if (profile.ReturnType != null && method.ReturnType != profile.ReturnType) return false;

            if (profile.ParameterTypes == null) return false;

            var parameters = method.GetParameters().Select(p => p.ParameterType).ToArray();
            if (parameters.Length != profile.ParameterTypes.Length) return false;
