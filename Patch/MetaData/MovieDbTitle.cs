...
namespace MediaInfoKeeper.Patch {
    ...
    public static class MovieDbTitle {
        ...
        private static readonly AsyncLocal<string> CurrentLookupLanguageCountryCode = new();

        private static readonly Regex ChineseRegex = new(@"[\u4E00-\u9FFF]", RegexOptions.Compiled);

        private static readonly Regex DefaultChineseEpisodeNameRegex = new(@"^第\s*\d+\s*集$", RegexOptions.Compiled);
        ...
        private static void OnAssemblyLoad(object sender, AssemblyLoadEventArgs args) {
            ...
            if (!string.Equals(name, "MovieDb", StringComparison.OrdinalIgnoreCase)) return;
        ...
        private static void TryInstallPatches(Assembly assembly) {
            try {
                ...
                patchCount += PatchMethod(genericMovieDbInfoProcessMainInfoMovie,
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(ProcessMainInfoMoviePrefix)));
                patchCount += PatchMethod(genericMovieDbInfoIsCompleteMovie,
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(IsCompletePrefix)),
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(IsCompletePostfix)));
                ...
                patchCount += PatchMethod(movieDbSeriesProviderImportData,
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(SeriesImportDataPrefix)));
                ...
                patchCount += PatchMethod(movieDbSeasonProviderImportData,
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(SeasonImportDataPrefix)));

                patchCount += PatchMethod(movieDbEpisodeProviderIsComplete,
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(IsCompletePrefix)),
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(IsCompletePostfix)));
                patchCount += PatchMethod(movieDbEpisodeProviderImportData,
                    new HarmonyMethod(typeof(MovieDbTitle), nameof(EpisodeImportDataPrefix)));
        ...
        private static void ResolveMethods(Assembly assembly) {
            ...
            var completeMovieDataParams = completeMovieData == null || tmdbSettingsResult == null
                ? null
                : new[] {
                    typeof(MetadataResult<Movie>), tmdbSettingsResult, typeof(string), completeMovieData, typeof(bool)
                };
            ...
            getTitleMovieData = FindInstanceMethod(completeMovieData, "GetTitle", Type.EmptyTypes, typeof(string));

            var movieDbProviderBase = assembly.GetType("MovieDb.MovieDbProviderBase", false);
            var episodeRootObject =
                movieDbProviderBase?.GetNestedType("EpisodeRootObject", BindingFlags.Public | BindingFlags.NonPublic);
            ...
            var seriesRootObject =
                movieDbSeriesProvider?.GetNestedType("SeriesRootObject", BindingFlags.Public | BindingFlags.NonPublic);
            ...
            movieDbSeriesProviderImportData = FindInstanceMethod(
                ...
                seriesRootObject == null || tmdbSettingsResult == null
                    ? null
                    : new[] {
                        typeof(MetadataResult<Series>), seriesRootObject, typeof(string), tmdbSettingsResult,
                        typeof(bool)
                    });
            ...
            getTitleSeriesInfo = FindInstanceMethod(seriesRootObject, "GetTitle", Type.EmptyTypes, typeof(string));

            var movieDbSeasonProvider = assembly.GetType("MovieDb.MovieDbSeasonProvider", false);
            var seasonRootObject =
                movieDbSeasonProvider?.GetNestedType("SeasonRootObject", BindingFlags.Public | BindingFlags.NonPublic);
            ...
            movieDbEpisodeProviderImportData = FindInstanceMethod(
                ...
                episodeRootObject == null || tmdbSettingsResult == null
                    ? null
                    : new[] {
                        typeof(MetadataResult<Episode>), typeof(EpisodeInfo), episodeRootObject, tmdbSettingsResult,
                        typeof(bool)
                    });
        }
        ...
        private static MethodInfo FindInstanceMethod(Type type, string name, Type[] parameterTypes,
            ...
            if (type == null || string.IsNullOrWhiteSpace(name) || parameterTypes == null) return null;
        ...
        private static void ProcessMainInfoMoviePrefix(
            ...
            if (!isEnabled || resultItem?.Item == null || movieData == null) return;
        ...
        private static bool IsCompletePrefix(BaseItem item, ref bool __result, out bool __state) {
            __state = false;
            if (!isEnabled || item == null) return true;
        ...
        private static void IsCompletePostfix(BaseItem item, ref bool __result, bool __state) {
            if (!isEnabled || !__state || item == null) return;

            if (BlockMovieDbNonFallbackLanguage(item.Overview)) {
                item.Overview = null;
                logger?.Debug("TMDB IsCompletePostfix: 清空非回退语言简介 item={0}",
                    item.FileName ?? item.Path ?? item.Id.ToString());
            }

            if (!string.IsNullOrWhiteSpace(item.Tagline)) {
                item.Tagline = null;
                logger?.Debug("TMDB IsCompletePostfix: 清空 Tagline item={0}",
                    item.FileName ?? item.Path ?? item.Id.ToString());
            }
        }
        ...
        private static void SeriesImportDataPrefix(
            ...
            object settings,
            bool isFirstLanguage) {
            if (!isEnabled || seriesResult?.Item == null || seriesInfo == null) return;
            ...
            try {
                ...
                if (IsUpdateNeeded(item.Name)) {
                    ...
                    if (!string.IsNullOrWhiteSpace(title)) item.Name = title;
                }

                var overview = GetPropertyString(seriesInfo, "overview");
                if (IsUpdateNeeded(item.Overview) && !string.IsNullOrWhiteSpace(overview)) item.Overview = DecodeOverview(overview);
        ...
        private static void EnsureSeriesInfoPostfix(
            ...
            Task __result) {
            if (!isEnabled || __result == null || WasCalledByMethod("FetchImages")) return;
            ...
            try {
                ...
                if (!IsChinese(name)) {
                    ...
                    if (alternativeTitles == null) return;
        ...
        private static bool TrySelectAlternativeSeriesTitle(
            ...
            selectedIso = null;
            selectedTitle = null;
            if (alternativeTitles == null) return false;
            ...
            foreach (var altTitle in alternativeTitles) {
                ...
                if (string.IsNullOrWhiteSpace(iso) || string.IsNullOrWhiteSpace(title)) continue;
            ...
            if (!isChineseLookup) return false;
        ...
        private static void SeasonImportDataPrefix(
            ...
            int seasonNumber,
            bool isFirstLanguage) {
            if (!isEnabled || item == null || seasonInfo == null) return;
        ...
        private static void EpisodeImportDataPrefix(
            ...
            object settings,
            bool isFirstLanguage) {
            if (!isEnabled || response == null || result?.Item == null) return;
            ...
            try {
                ...
                if (!string.IsNullOrWhiteSpace(nameValue)) item.Name = nameValue;
                ...
                if (!string.IsNullOrWhiteSpace(decodedOverview)) item.Overview = decodedOverview;

                var overviewPreview = decodedOverview ?? string.Empty;
                if (overviewPreview.Length > 30) overviewPreview = overviewPreview.Substring(0, 30) + "...";
        ...
        private static void MetadataLanguagesPostfix(
            ...
            ref string[] __result) {
            if (!isEnabled || searchInfo == null) return;
            ...
            try {
                ...
                foreach (var fallbackLanguage in fallbackLanguages) {
                    if (list.Contains(fallbackLanguage, StringComparer.OrdinalIgnoreCase)) continue;
        ...
        private static void GetImageLanguagesParamPostfix(ref string __result) {
            if (!isEnabled || string.IsNullOrWhiteSpace(__result)) return;
        ...
        private static void ReplaceCnGenreName(object seriesInfo, string from, string to) {
            ...
            foreach (var genre in genres) {
                ...
                if (string.Equals(name, from, StringComparison.OrdinalIgnoreCase)) SetPropertyValue(genre, "name", to);
            }
        }

        private static string MapLanguageToProviderLanguage(object instance, string language,
            string[] providerLanguages) {
            if (instance == null || mapLanguageToProviderLanguage == null) return null;
        ...
        private static bool IsUpdateNeeded(string currentValue, string newEpisodeName = null) {
            ...
            if (!isEpisodeName) return !IsChinese(currentValue);
        ...
        private static List<string> GetMovieDbFallbackLanguages() {
            ...
            if (string.IsNullOrWhiteSpace(configured)) return new List<string> { "zh-sg" };
            ...
            if (ordered.Count == 0) ordered.Add("zh-sg");
        ...
        private static MetaDataOptions GetTmdbOptions() {
            ...
            if (plugin == null) return new MetaDataOptions();
        ...
        private static bool WasCalledByMethod(string methodName) {
            if (movieDbAssembly == null || string.IsNullOrWhiteSpace(methodName)) return false;
            ...
            try {
                ...
                foreach (var frame in frames) {
                    ...
                    if (method.DeclaringType?.Assembly == movieDbAssembly) return true;
                }
            }
            catch {
                // ignore
