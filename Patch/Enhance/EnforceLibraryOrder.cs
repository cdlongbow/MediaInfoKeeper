...
namespace MediaInfoKeeper.Patch {
    ...
    public static class EnforceLibraryOrder {
        ...
        private static void Patch() {
            if (isPatched || harmony == null || getUserViews == null) return;

            harmony.Patch(getUserViews, new HarmonyMethod(typeof(EnforceLibraryOrder), nameof(GetUserViewsPrefix)));
            isPatched = true;
        }
        ...
        private static bool GetUserViewsPrefix(User user) {
            if (!isEnabled || user?.Configuration == null) return true;

            var adminOrderedViews = Plugin.LibraryService?.GetAdminOrderedViews();
            if (adminOrderedViews == null || adminOrderedViews.Length == 0) return true;
