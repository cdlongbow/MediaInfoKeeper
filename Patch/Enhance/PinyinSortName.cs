...
namespace MediaInfoKeeper.Patch {
    ...
    public static class PinyinSortName {
        private static readonly HashSet<char> ValidPrefixChars = new("#ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        ...
        private static void CreateSortNamePostfix(BaseItem __instance, ref ReadOnlySpan<char> __result) {
            if (!isEnabled || __instance == null) return;
            ...
            if (!LanguageUtility.IsChinese(currentSortName)) return;

            var pinyinInitials = BuildPinyinSortName(__instance, currentSortName);
            if (string.IsNullOrWhiteSpace(pinyinInitials)) return;
        ...
        private static void GetPrefixesPostfix(ref object __result) {
            if (!isEnabled || !(__result is NameValuePair[] pairs)) return;
            ...
            if (normalizedPairs.Length > 0) __result = normalizedPairs;
        }
        ...
        private static NameValuePair NormalizePrefixPair(NameValuePair pair) {
            ...
            if (pair == null) return new NameValuePair { Name = normalizedPrefix };
        ...
        private static string NormalizePrefix(string prefix) {
            ...
            if (trimmedPrefix.Length == 1 && ValidPrefixChars.Contains(trimmedPrefix[0])) return trimmedPrefix;

            var leadingCharacter = GetLeadingCharacter(trimmedPrefix);
            if (leadingCharacter == null) return null;
            ...
            if (LanguageUtility.IsChinese(leadingCharacter))
                try {
                    ...
                    if (firstLetter != null) return firstLetter;
                }
                catch (Exception ex) {
        ...
        private static string ExtractFirstValidPrefix(string value) {
            ...
            foreach (var character in value.Trim()) {
                if (char.IsLetter(character)) {
                    ...
                    if (upper >= 'A' && upper <= 'Z') return upper.ToString();
                }

                if (char.IsDigit(character) || char.IsPunctuation(character)) return "#";
            }
        ...
        public static bool RebuildSortName(BaseItem item) {
            if (item == null || item.IsFieldLocked(MetadataFields.SortName)) return false;
        ...
        private static string BuildPinyinSortName(BaseItem item, string source) {
            if (!LanguageUtility.IsChinese(source)) return null;
