...
namespace MediaInfoKeeper.Patch {
    ...
    public static class IsoProbeSupport {
        ...
        private static void IsSupportedPostfix([HarmonyArgument(0)] string path, ref bool __result) {
            ...
            if (!IsoFfprobeRouting.LooksLikeIsoPath(path)) return;
    ...
    public static class IsoProbeInput {
        ...
        private static readonly ConditionalWeakTable<object, StrongBox<string>> tempProbeInputs = new();
        ...
        public static void Initialize(ILogger pluginLogger, bool enabled) {
            ...
            try {
                ...
                var mediaProbeManagerType =
                    mediaEncodingAssembly?.GetType("Emby.Server.MediaEncoding.Probing.MediaProbeManager");
                var mediaInfoRequestType =
                    controllerAssembly?.GetType("MediaBrowser.Controller.MediaEncoding.MediaInfoRequest");
                ...
                harmony.Patch(
                    getMediaInfoMethod,
                    new HarmonyMethod(typeof(IsoProbeInput), nameof(GetMediaInfoPrefix)),
                    new HarmonyMethod(typeof(IsoProbeInput), nameof(GetMediaInfoPostfix)));
            }
            catch (Exception ex) {
        ...
        private static void GetMediaInfoPrefix(object __instance, [HarmonyArgument(0)] object request,
            [HarmonyArgument(1)] CancellationToken cancellationToken) {
            if (__instance == null || request == null) return;
            ...
            try {
                ...
                if (string.Equals(remappedPath, path, StringComparison.OrdinalIgnoreCase)) return;
        ...
        private static void GetMediaInfoPostfix([HarmonyArgument(0)] object request, ref object __result) {
            if (request == null || __result == null || mediaInfoType == null) return;

            var expectedTaskType = typeof(Task<>).MakeGenericType(mediaInfoType);
            if (!expectedTaskType.IsInstanceOfType(__result)) return;

            var wrapMethod =
                typeof(IsoProbeInput).GetMethod(nameof(WrapTask), BindingFlags.Static | BindingFlags.NonPublic);
            var genericWrapMethod = wrapMethod?.MakeGenericMethod(mediaInfoType);
            if (genericWrapMethod == null) return;
        ...
        private static void ApplyBlurayPlaylistMetadata(ModelMediaInfo mediaInfo, object request) {
            ...
            if (!IsoFfprobeRouting.LooksLikeIsoPath(sourcePath)) return;

            try {
                using var discInfo = IsoDiscProbeInput.ReadDiscInfo(
                    sourcePath,
                    message => logger?.Warn(message));
                if (discInfo?.Playlist == null) return;
        ...
        private static void ApplyDiscSourceMetadata(ModelMediaInfo mediaInfo, IsoDiscProbeInput.IsoDiscInfo discInfo) {
            if (mediaInfo == null || discInfo?.Playlist == null) return;
            ...
            if (discInfo.SourceLength.HasValue && discInfo.SourceLength.Value > 0) mediaInfo.Size = discInfo.SourceLength.Value;
        }

        private static void ApplyDiscStreamMetadata(ModelMediaInfo mediaInfo, BlurayPlaylistInfo playlistInfo) {
            if (mediaInfo == null || playlistInfo?.Streams == null || playlistInfo.Streams.Count == 0) return;

            if (mediaInfo.MediaStreams == null) mediaInfo.MediaStreams = new List<MediaStream>();

            var currentAudios = mediaInfo.MediaStreams.Where(stream => stream?.Type == MediaStreamType.Audio).ToList();
            var currentSubtitles = mediaInfo.MediaStreams.Where(stream => stream?.Type == MediaStreamType.Subtitle)
                .ToList();
            ...
            var added = 0;
            added += AppendMissingStreams(mediaInfo.MediaStreams, currentAudios.Count, discAudios,
                MediaStreamType.Audio);
            added += AppendMissingStreams(mediaInfo.MediaStreams, currentSubtitles.Count, discSubtitles,
                MediaStreamType.Subtitle);

            if (added > 0) ReindexStreams(mediaInfo.MediaStreams);
        ...
        private static int ApplyDiscStreamsByOrder(
            ...
            if (currentStreams == null || discStreams == null) return 0;

            var changed = 0;
            var limit = Math.Min(currentStreams.Count, discStreams.Count);
            for (var i = 0; i < limit; i++) changed += ApplyDiscStreamMetadata(currentStreams[i], discStreams[i], streamType);
        ...
        private static int RemoveExpandedCompatibilityAudio(
            ...
            if (mediaStreams == null || currentAudios == null || discAudios == null) return 0;
        ...
        private static int AppendMissingStreams(
            ...
            List<BlurayPlaylistStream> discStreams,
            MediaStreamType streamType) {
            if (mediaStreams == null || discStreams == null || existingCount >= discStreams.Count) return 0;

            var added = 0;
            for (var i = existingCount; i < discStreams.Count; i++) {
                var discStream = discStreams[i];
                if (discStream == null) continue;
        ...
        private static int ApplyDiscStreamMetadata(
            ...
            BlurayPlaylistStream discStream,
            MediaStreamType streamType) {
            if (mediaStream == null || discStream == null) return 0;
        ...
        private static string NormalizeDiscCodec(string codec, MediaStreamType streamType) {
            if (streamType == MediaStreamType.Subtitle) {
                if (string.Equals(codec, "pgs", StringComparison.OrdinalIgnoreCase)) return "PGSSUB";
        ...
        private static void ApplyDiscChapters(ModelMediaInfo mediaInfo, BlurayPlaylistInfo playlistInfo) {
            if (mediaInfo.Chapters != null && mediaInfo.Chapters.Length > 0) return;
        ...
        private static void ApplyDolbyVisionFallback(ModelMediaInfo mediaInfo, object request) {
            ...
            if (videoStreams.Count == 0) return;

            var mainVideo = videoStreams[0];
            if (mainVideo.ExtendedVideoType == ExtendedVideoTypes.DolbyVision) return;

            if (!LooksLikeHdrBase(mainVideo)) return;

            var inferredSubtype = GetDolbyVisionSubtype(videoStreams, out var reason);
            if (!inferredSubtype.HasValue) return;
        ...
        private static bool IsEnhancementLayerCandidate(MediaStream stream) {
            if (stream == null || stream.Type != MediaStreamType.Video) return false;
        ...
        private static string GetPreferredDiscSourcePath(ModelMediaInfo mediaInfo, object request) {
            ...
            if (mediaSource != null) {
                ...
                if (IsoFfprobeRouting.LooksLikeIsoPath(requestIsoPath)) return requestIsoPath;
            }
        ...
        private static void CleanupTempProbeInput(object request) {
            if (request == null || !tempProbeInputs.TryGetValue(request, out var probeInput)) return;

            tempProbeInputs.Remove(request);
            try {
                if (File.Exists(probeInput.Value)) File.Delete(probeInput.Value);
            }
            catch (Exception ex) {
        ...
        private static string ResolveMediaSourceIsoPath(object mediaSource) {
            ...
            if (IsoFfprobeRouting.LooksLikeIsoPath(path)) return path;

            var probePath = GetStringProperty(mediaSource, "ProbePath");
            if (IsoFfprobeRouting.LooksLikeIsoPath(probePath)) return probePath;
    ...
    internal static class IsoDiscProbeInput {
        ...
        public static async Task<string> PrepareProbeInputAsync(
            ...
            if (probeBytes <= 0) probeBytes = DefaultProbeBytes;

            using var discInfo = await ReadDiscInfoAsync(path, cancellationToken, log).ConfigureAwait(false);
            if (discInfo?.Playlist == null) return path;

            var targetClips = discInfo.ClipPaths;
            if (targetClips.Count == 0) {
                log?.Invoke(
                    $"ISO playlist 探测输入跳过：未找到主 clip path={path} playlist={discInfo.Playlist?.PlaylistName ?? "<null>"}");
                return path;
            }
        ...
        private static string GetEmbyCachePath() {
            ...
            if (string.IsNullOrWhiteSpace(cachePath)) throw new InvalidOperationException("Emby CachePath is not available.");

            return cachePath;
        }

        public static IsoDiscInfo ReadDiscInfo(string path, Action<string> log = null) {
            if (!IsoFfprobeRouting.LooksLikeIsoPath(path)) return null;
        ...
        private static async Task<Stream> OpenSourceStreamAsync(
            string path,
            CancellationToken cancellationToken) {
            if (!IsHttpIsoPath(path)) return File.OpenRead(path);
        ...
        private static bool IsHttpIsoPath(string path) {
            if (!IsoFfprobeRouting.LooksLikeIsoPath(path)) return false;
        ...
        private static async Task<long> CopyUpToAsync(Stream input, Stream output, int byteLimit,
            CancellationToken cancellationToken) {
        ...
        private sealed class HttpRangeStream : Stream {
            private readonly Dictionary<long, byte[]> blockCache = new();
            ...
            private long FetchLength() {
                ...
                if (response.Content.Headers.ContentRange?.Length is long contentLength) return contentLength;
            ...
            private byte[] GetOrFetchBlock(long blockStart) {
                if (blockCache.TryGetValue(blockStart, out var cached)) return cached;
