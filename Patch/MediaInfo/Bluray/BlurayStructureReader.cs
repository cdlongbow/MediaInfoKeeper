...
namespace MediaInfoKeeper.Patch.MediaInfo.Bluray {
    internal static class BlurayStructureReader {
        ...
        public static BlurayPlaylistInfo ReadMainPlaylist(string path, double? preferredDurationSeconds = null) {
            ...
            if (path.EndsWith(".iso", StringComparison.OrdinalIgnoreCase)) return ReadFromIso(path, preferredDurationSeconds);

            return ReadFromDirectory(path, preferredDurationSeconds);
        }

        internal static BlurayPlaylistInfo ReadMainPlaylistFromFileSystem(dynamic fileSystem,
            double? preferredDurationSeconds = null) {
        ...
        internal static List<string> ResolvePlaylistClipPaths(dynamic fileSystem, BlurayPlaylistInfo playlist) {
            var clipNames = (playlist?.ClipFileNames ?? new List<string>())
                ...
                .ToList();
            if (clipNames.Count == 0) return new List<string>();

            var streamDir = ResolveStreamDirectory(fileSystem);
            if (streamDir == null) return new List<string>();
            ...
            foreach (var clipName in clipNames) {
                ...
                try {
                    ...
                    if (info.Exists) resolved.Add(candidate);
                }
                catch {
        ...
        private static BlurayPlaylistInfo ReadFromDirectory(string path, double? preferredDurationSeconds) {
            ...
            if (string.IsNullOrWhiteSpace(discRoot)) return null;

            var playlistDir = Path.Combine(discRoot, "BDMV", "PLAYLIST");
            if (!Directory.Exists(playlistDir)) return null;
            ...
            foreach (var file in Directory.EnumerateFiles(playlistDir, "*.mpls", SearchOption.TopDirectoryOnly)) {
                ...
                if (IsBetterMainPlaylist(playlist, best, preferredDurationSeconds)) best = playlist;
            }
        ...
        private static BlurayPlaylistInfo ReadFromFileSystem(dynamic fileSystem, double? preferredDurationSeconds) {
            ...
            if (playlistDir == null) return null;
        ...
        private static bool IsBetterMainPlaylist(BlurayPlaylistInfo candidate, BlurayPlaylistInfo currentBest,
            ...
            if (preferredDurationSeconds.HasValue) {
                ...
                if (Math.Abs(candidateDelta - currentDelta) > MainPlaylistDurationToleranceSeconds) return candidateDelta < currentDelta;
            }
            ...
            if (candidateLooksLikeMovie != bestLooksLikeMovie) return candidateLooksLikeMovie;
            ...
            if (durationDelta > MainPlaylistDurationToleranceSeconds) return true;

            if (durationDelta > 0) return true;
        ...
        private static string ResolvePlaylistDirectory(dynamic fileSystem) {
            ...
            foreach (var candidate in candidates)
                try {
                    if (fileSystem.DirectoryExists(candidate)) return candidate;
                }
                catch {
        ...
        private static string NormalizeDirectoryRoot(string path) {
            ...
            if (normalized.EndsWith("/BDMV", StringComparison.OrdinalIgnoreCase)) return path.Substring(0, path.Length - 5);
