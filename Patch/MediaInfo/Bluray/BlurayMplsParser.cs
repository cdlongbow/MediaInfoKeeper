        ...
        public static BlurayPlaylistInfo Parse(string playlistName, byte[] data) {
            ...
            for (var i = 0; i < clipCount; i++) {
                ...
                if (multiAngle > 0) {
                    ...
                    for (var j = 0; j < angleCount - 1; j++) {
                        ...
                        if (!string.IsNullOrWhiteSpace(angleClipFileName) &&
                            !string.IsNullOrWhiteSpace(angleClipFileType))
            ...
            for (var i = 0; i < chapterCount; i++) {
                if (data[pos + 1] == 1) {
                    ...
                    var chapterTimestamp =
                        NormalizeTimestamp(unchecked((int)BinaryPrimitives.ReadUInt32BigEndian(data.AsSpan(pos + 4))));
                    if (clipIndex >= 0 && clipIndex < clipLengths.Count) {
                        ...
                        if (result.TotalLengthSeconds - relativeSeconds > 1.0) result.Chapters.Add(relativeSeconds);
                    }
                }
