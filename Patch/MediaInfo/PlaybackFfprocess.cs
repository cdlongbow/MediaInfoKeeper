...
namespace MediaInfoKeeper.Patch {
    ...
    public static class PlaybackFfprocess {
        ...
        public static void Initialize(ILogger pluginLogger, bool enabled) {
            ...
            try {
                ...
                var mediaSourceManagerType =
                    serverImplementations?.GetType("Emby.Server.Implementations.Library.MediaSourceManager");

                if (mediaInfoServiceType == null || getPostedPlaybackInfoType == null) {
                    PatchLog.InitFailed(logger, nameof(PlaybackFfprocess),
                ...
                harmony.Patch(
                    playbackInfoEntry,
                    new HarmonyMethod(typeof(PlaybackFfprocess), nameof(PlaybackInfoPrefix)),
                    new HarmonyMethod(typeof(PlaybackFfprocess), nameof(AsyncPostfix)));
                harmony.Patch(
                    openLiveStreamEntry,
                    new HarmonyMethod(typeof(PlaybackFfprocess), nameof(OpenLiveStreamPrefix)),
                    new HarmonyMethod(typeof(PlaybackFfprocess), nameof(AsyncPostfix)));
            }
            catch (Exception ex) {
        ...
        private static FfProcessGuard.AllowanceContext CreatePlaybackContext(long? itemId) {
            BaseItem item = null;
            if (itemId.GetValueOrDefault() > 0) item = Plugin.LibraryManager?.GetItemById(itemId.Value);
