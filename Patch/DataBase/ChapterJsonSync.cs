...
namespace MediaInfoKeeper.Patch {
    ...
    public static class ChapterJsonSync {
        private static readonly AsyncLocal<bool> SkipPersist = new();
        ...
        private static readonly AsyncLocal<int> AllowClearPersistCount = new();
        ...
        private static void SaveChaptersPostfix(long itemId, List<ChapterInfo> chapters, bool __runOriginal) {
            if (__runOriginal && chapters?.Count > 0) PersistSavedChapters(itemId);
        }

        [HarmonyPostfix]
        private static void DeleteChaptersPostfix(long itemId, bool __runOriginal) {
            if (__runOriginal && AllowClearPersistCount.Value > 0) PersistSavedChapters(itemId);
        }
