...
namespace MediaInfoKeeper.External {
    ...
    public static class MovieDbSeriesProvider {
        ...
        [field: ThreadStatic] public static string CurrentSeriesContainingFolderPath { get; set; }
        ...
        public static async Task<RemoteSearchResult[]> RewriteMissingEpisodesAsync(
            ...
            if (fallbackTask == null) return Array.Empty<RemoteSearchResult>();
            ...
            if (options?.EnableMissingEpisodesEnhance != true) return fallbackResults;

            var tmdbId = searchInfo?.GetProviderId(MetadataProviders.Tmdb);
            if (string.IsNullOrWhiteSpace(tmdbId)) return fallbackResults;
            ...
            if (options.EnableLocalEpisodeGroup &&
                ...
                try {
                    ...
                    if (!string.IsNullOrWhiteSpace(raw)) episodeGroup = JsonSerializer.Deserialize<EpisodeGroupResponse>(raw, JsonOptions);
                }
                catch (Exception ex) {
            ...
            if (episodeGroup?.groups == null || episodeGroup.groups.Count == 0) {
                ...
                if (existingEpisodeNumbers != null && fallbackResults.Length > 0) {
                    ...
                    normalizedFallbackResults = fallbackResults.Select(result => {
                        if (result?.ParentIndexNumber == null || result.IndexNumber == null) return result;

                        if (existingEpisodeNumbers.Contains((result.ParentIndexNumber.Value,
            ...
            foreach (var episode in (group.episodes ?? Enumerable.Empty<GroupEpisode>()).OrderBy(e => e.order)) {
                ...
                if (episode.id > 0) providerIds[MetadataProviders.Tmdb.ToString()] = episode.id.ToString();
            ...
            logger?.Debug(
                ...
                string.IsNullOrWhiteSpace(episodeGroup.id)
                    ? string.IsNullOrWhiteSpace(episodeGroupId) ? "<empty>" : episodeGroupId
                    : episodeGroup.id,
                results.Count,
            ...
            try {
                ...
                if (Plugin.Instance?.Options?.MetaData?.EnableLocalEpisodeGroup == true &&
                    ...
                    try {
                        ...
                        File.WriteAllText(localEpisodeGroupPath,
                            JsonSerializer.Serialize(data, WriteIndentedJsonOptions));
                    }
                    catch (Exception ex) {
