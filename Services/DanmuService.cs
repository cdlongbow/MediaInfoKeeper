...
namespace MediaInfoKeeper.Services {
    public class DanmuService {
        ...
        private readonly ConcurrentQueue<QueuedDanmuItem> itemAddedQueue = new();
        ...
        public DanmuService(ILogManager logManager, IHttpClient httpClient) {
            logger = Plugin.SharedLogger ?? logManager.GetLogger(Plugin.PluginName);
        ...
        public Task<bool> QueueDownloadAsync(long internalId, bool overwriteExisting,
            CancellationToken cancellationToken) {
            var completionSource = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
            if (cancellationToken.CanBeCanceled) cancellationToken.Register(() => completionSource.TrySetCanceled(cancellationToken));

            var item = Plugin.LibraryManager?.GetItemById(internalId);
            if (!overwriteExisting && ShouldSkipAutoDownload(item)) {
                logger.Debug($"弹幕下载: 跳过 {item?.FileName} 文件已存在");
        ...
        public Task<DanmuDownloadResult> QueueDownloadWithReasonAsync(long internalId, bool overwriteExisting,
            CancellationToken cancellationToken) {
            var completionSource =
                new TaskCompletionSource<DanmuDownloadResult>(TaskCreationOptions.RunContinuationsAsynchronously);
            if (cancellationToken.CanBeCanceled) cancellationToken.Register(() => completionSource.TrySetCanceled(cancellationToken));
        ...
        private void Enqueue(long internalId, bool overwriteExisting, TaskCompletionSource<bool> completionSource) {
            ...
            if (Interlocked.CompareExchange(ref queueWorkerStarted, 1, 0) == 0) _ = Task.Run(ProcessItemAddedQueueAsync);
        }

        private void Enqueue(long internalId, bool overwriteExisting,
            TaskCompletionSource<DanmuDownloadResult> detailCompletionSource) {
            itemAddedQueue.Enqueue(new QueuedDanmuItem {
                InternalId = internalId,
                OverwriteExisting = overwriteExisting,
        ...
        private async Task ProcessItemAddedQueueAsync() {
            while (true)
                try {
                    ...
                    if (!itemAddedQueue.TryDequeue(out var queuedItem) || queuedItem == null) continue;
                    ...
                    try {
                        ...
                        logger.Debug($"弹幕下载: 失败 {queuedItem.InternalId} {ex.Message}");
                    ...
                    logger.Debug($"弹幕下载: 失败 queue {ex.Message}");
                    logger.Debug(ex.StackTrace);
                }
        }

        private async Task<DanmuDownloadResult> ProcessQueuedItemAsync(QueuedDanmuItem queuedItem) {
            if (queuedItem == null) return Failed("队列项为空");

            var internalId = queuedItem.InternalId;
            var currentItem = Plugin.LibraryManager?.GetItemById(internalId);
            if (currentItem == null || Plugin.Instance == null ||
                Plugin.Instance.Options.MainPage?.PlugginEnabled != true || !IsSupportedItem(currentItem))
                return Failed("条目无效或插件未启用");

            if (!IsEnabled) return Failed("弹幕 API 未启用");

            return await TryDownloadDanmuXmlDetailedAsync(currentItem, CancellationToken.None,
                queuedItem.OverwriteExisting).ConfigureAwait(false);
        }

        public bool ShouldSkipAutoDownload(BaseItem item) {
            if (item == null) return true;

            if (item is not Episode && item is not Movie) return true;

            if (ShouldAlwaysFetchLatest()) return false;
        ...
        public async Task<bool> TryDownloadDanmuXmlAsync(BaseItem item, CancellationToken cancellationToken,
            bool overwriteExisting = false) {
            var result = await TryDownloadDanmuXmlDetailedAsync(item, cancellationToken, overwriteExisting)
                .ConfigureAwait(false);
            return result.Succeeded;
        }

        public async Task<DanmuDownloadResult> TryDownloadDanmuXmlDetailedAsync(BaseItem item,
            CancellationToken cancellationToken, bool overwriteExisting = false) {
            if (!IsEnabled || item == null) return Failed("弹幕 API 未启用或条目为空");

            if (item is not Episode && item is not Movie) return Failed("条目类型不支持");
            ...
            if (!overwrite && File.Exists(targetPath)) return Failed("已存在弹幕文件");
            ...
            if (fetchResult.XmlBytes == null || fetchResult.XmlBytes.Length == 0) {
                if (!string.IsNullOrWhiteSpace(fetchResult.Reason)) return Failed(fetchResult.Reason);
            ...
            logger.Info($"Danmu 下载成功: {FormatItemForLog(item)}");
            return Succeeded();
        }
        ...
        public async Task<DanmuFetchResult> FetchDanmuXmlDetailedForApiAsync(BaseItem item,
            CancellationToken cancellationToken) {
        ...
        private async Task<DanmuFetchResult> FetchDanmuXmlDetailedAsync(BaseItem item,
            CancellationToken cancellationToken) {
            if (!IsEnabled || item == null) return new DanmuFetchResult { Reason = "弹幕 API 未启用或条目为空" };
        ...
        private async Task<byte[]> DownloadXmlAsync(string baseUrl, string episodeId,
            CancellationToken cancellationToken) {
            ...
            if ((int)response.StatusCode < 200 || (int)response.StatusCode >= 300) {
                ...
                logger.Debug(
                    $"弹幕下载: 失败 {episodeId} status={(int)response.StatusCode} url={requestUrl} body={body}");
                return null;
            }
        ...
        private static string BuildApiUrl(string baseUrl, string relativePath) {
            ...
            if (!normalizedBaseUrl.EndsWith("/", StringComparison.Ordinal)) normalizedBaseUrl += "/";
