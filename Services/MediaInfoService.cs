...
namespace MediaInfoKeeper.Services {
    public class MediaInfoService {
        ...
        public bool HasMediaInfo(BaseItem item) {
            if (item is not IHasMediaSources || !item.RunTimeTicks.HasValue) return false;

            return mediaSourceManager.GetMediaStreams(item.InternalId)
        ...
        public List<MediaSourceInfo> GetStaticMediaSources(BaseItem item, bool enableAlternateMediaSources) {
            if (item is not IHasMediaSources) return new List<MediaSourceInfo>();

            var collectionFolders = libraryManager.GetCollectionFolders(item).Cast<BaseItem>().ToArray();
        ...
        private MetadataRefreshOptions GetMediaInfoRefreshOptions() {
            var options = new MetadataRefreshOptions(new DirectoryService(logger, fileSystem));
        ...
        public MediaInfoDocument.MediaInfoRestoreResult RestorePersistedMediaInfo(BaseItem item) {
            if (item is not Video && item is not Audio) return MediaInfoDocument.MediaInfoRestoreResult.Failed;

            var result = Plugin.MediaSourceInfoStore.ApplyToItem(item);
            if (item is Video)
                Plugin.ChaptersStore.ApplyToItem(item);
            else if (item is Audio) Plugin.EmbeddedInfoStore.ApplyToItem(item);

            return result;
        }

        public MediaInfoDocument.MediaInfoRestoreResult RestorePersistedMediaInfoForExistingSource(BaseItem item,
            string source) {
            var displayName = item?.FileName ?? item?.Path ?? item?.Name;
            if (item is not Video && item is not Audio) {
                logger.Info($"{source} 跳过 条目非音视频: {displayName}");
            ...
            using (FfProcessGuard.Allow()) {
                ...
                if (string.IsNullOrEmpty(filePath)) {
                    logger.Info($"{source} 跳过 无路径: {displayName}");
                    return MediaInfoDocument.MediaInfoRestoreResult.Failed;
                }

                if (IsMissingLocalFile(filePath, GetMediaInfoRefreshOptions().DirectoryService)) {
                    logger.Info($"{source} 跳过 文件不存在: {displayName}");
                    return MediaInfoDocument.MediaInfoRestoreResult.Failed;
                }
        ...
        internal async Task<bool> ExtractMediaInfoAsync(
            ...
            for (var attempt = 1; attempt <= maxAttempts; attempt++) {
                ...
                if (HasRequiredMediaInfo(item, requiredStreamTypes)) {
                    logger.Info($"{source} 提取媒体信息队列出队 已存在媒体流: {displayName}");
                    return true;
                }
                ...
                if (result == MediaInfoExtractionAttemptResult.Succeeded) return true;

                if (result == MediaInfoExtractionAttemptResult.NotRetryable) return false;
            }
        ...
        private async Task<MediaInfoExtractionAttemptResult> ExtractMediaInfoOnceAsync(
            ...
            CancellationToken cancellationToken) {
            if (!(item is Video) && !(item is Audio)) return MediaInfoExtractionAttemptResult.NotRetryable;

            using (FfProcessGuard.Allow()) {
                var filePath = item.Path;
                if (string.IsNullOrEmpty(filePath)) {
                    logger.Info($"{source} 提取媒体信息跳过 无路径: {displayName}");
                ...
                if (IsMissingLocalFile(filePath, directoryService)) {
                    logger.Info($"{source} 提取媒体信息跳过 文件不存在: {displayName}");
                    return MediaInfoExtractionAttemptResult.NotRetryable;
                }
                ...
                if (deserializeResult == MediaInfoDocument.MediaInfoRestoreResult.Restored ||
                    ...
                    logger.Info($"{source} 提取媒体信息恢复后仍无媒体流，继续刷新: {displayName}");
                }

                var startAttemptSuffix =
                    maxAttempts > 1 && attempt > 1 ? $" 第 {attempt}/{maxAttempts} 次" : string.Empty;
                logger.Info($"{source} 提取开始{startAttemptSuffix}: {displayName}");
                ...
                if (!HasRequiredMediaInfo(item, requiredStreamTypes)) return MediaInfoExtractionAttemptResult.RetryableFailure;

                logger.Info($"{source} 提取成功: {displayName}");
                return MediaInfoExtractionAttemptResult.Succeeded;
            }
        }

        private bool HasRequiredMediaInfo(BaseItem item, MediaStreamType[] requiredStreamTypes) {
            if (requiredStreamTypes == null || requiredStreamTypes.Length == 0) return HasMediaInfo(item);
