...
namespace MediaInfoKeeper.Services {
    internal static class NetworkProbe {
        ...
        public static Task<Result> RunProxyLatencyAsync(NetWorkOptions options) {
            if (options == null) return Task.FromResult(Build(ItemStatus.Unavailable, "不可用", "N/A"));

            if (!options.EnableProxyServer) return Task.FromResult(Build(ItemStatus.Unavailable, "不可用", "N/A"));

            if (!TryParseProxyEndpoint(options.ProxyServerUrl, out var scheme, out var host, out var port,
        ...
        public static async Task<Result> RunTmdbAltAsync(NetWorkOptions options) {
            if (options == null) return Build(ItemStatus.Unavailable, "不可用", "N/A");
            ...
            if (!hasApiOverride && !hasImageOverride) return Build(ItemStatus.Unavailable, "未启用", "N/A");
            ...
            if (hasApiOverride) {
                ...
                var apiResult = await ProbeTmdbApiAsync(apiBaseUrl, options.AlternativeTmdbApiKey)
            ...
            if (hasFailure && hasSuccess) return Build(ItemStatus.Warning, "部分可用", statusText);

            if (hasSuccess) return Build(ItemStatus.Succeeded, "可用", statusText);
        ...
        private static async Task<Result> ProbeProxyCoreAsync(
            ...
            try {
                ...
                if (ignoreCertificateValidation) handler.ServerCertificateCustomValidationCallback = (_, _, _, _) => true;
        ...
        private static Task<(bool Succeeded, HttpStatusCode StatusCode, string StatusText)> ProbeTmdbApiAsync(
            string baseUrl, string apiKey) {
            var url = CombineUrl(baseUrl, "/3/configuration");
            if (!string.IsNullOrWhiteSpace(apiKey)) url += "?api_key=" + Uri.EscapeDataString(apiKey.Trim());

            return ProbeHttpAsync("GET", url, HttpStatusCode.Unauthorized);
        }

        private static async Task<(bool Succeeded, HttpStatusCode StatusCode, string StatusText)> ProbeTmdbImageAsync(
            string baseUrl) {
        ...
        private static async Task<(bool Succeeded, HttpStatusCode StatusCode, string StatusText)> ProbeHttpAsync(
            ...
            try {
                ...
                if (httpClient == null) return (false, default, url + " IHttpClient 不可用");
