...
namespace MediaInfoKeeper.Services {
    public static class IntroScanRunner {
        ...
        private static readonly Queue<IntroScanRequest> HighestScanQueue = new();

        private static readonly Queue<IntroScanRequest> HighScanQueue = new();

        private static readonly Queue<IntroScanRequest> NormalScanQueue = new();

        private static readonly Queue<IntroScanRequest> LowScanQueue = new();

        private static readonly Dictionary<long, IntroScanRequest> InFlightScans = new();
        ...
        public static Task ScanEpisodesAsync(IReadOnlyList<Episode> episodes, CancellationToken cancellationToken,
            ...
            foreach (var episode in episodes) _ = ScanEpisodeAsync(episode, "计划任务片头扫描", cancellationToken);
        ...
        public static Task<bool> ScanEpisodeAsync(
            ...
            if (episode == null) return Task.FromResult(false);
        ...
        private static Queue<IntroScanRequest> GetNextQueueInsideLock() {
            if (HighestScanQueue.Count > 0) return HighestScanQueue;

            if (HighScanQueue.Count > 0) return HighScanQueue;
        ...
        private static async Task<bool> RunEpisodeScanAsync(
            ...
            if (!Plugin.MediaInfoService.HasMediaInfo(episode)) {
                var extracted = await MediaInfoRunner
                    .ExtractMediaInfoAsync(episode.InternalId, preExtractSource, cancellationToken,
            ...
            try {
                ...
                if (runtime?.ManagerType == null) {
                    ...
                    if (detector == null) {
                        ...
                        if (await RunAudioFingerprintWorkflowAsync(detector, episode, cancellationToken)
                                .ConfigureAwait(false))
                            return true;
        ...
        private static AudioFingerprintRuntime GetOrCreateAudioFingerprintRuntime() {
            if (audioFingerprintRuntime != null) return audioFingerprintRuntime;

            lock (runtimeLock) {
                if (audioFingerprintRuntime != null) return audioFingerprintRuntime;

                var providersAssembly = Assembly.Load("Emby.Providers");
                if (providersAssembly == null) return null;
                ...
                var seasonFingerprintInfoType =
                    providersAssembly.GetType("Emby.Providers.Markers.SeasonFingerprintInfo");
                ...
                var getAllFingerprintFilesForSeason = PatchMethodResolver.Resolve(
                    ...
                    new MethodSignatureProfile {
                        ...
                        BindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                        ParameterTypes = new[] {
                            typeof(Season), typeof(Episode[]), typeof(LibraryOptions), typeof(IDirectoryService),
                            typeof(CancellationToken)
                        }
                    },
                ...
                var updateSequencesForSeason = PatchMethodResolver.Resolve(
                    ...
                    new MethodSignatureProfile {
                        ...
                        BindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                        ParameterTypes = new[] {
                            typeof(Season), seasonFingerprintInfoType, typeof(Episode), typeof(LibraryOptions),
        ...
        private static async Task<bool> RunAudioFingerprintWorkflowAsync(
            ...
            CancellationToken cancellationToken) {
            logger.Debug(
                $"AudioFingerprint workflow start: detector={detector.GetType().FullName}, item={episode?.Path ?? episode?.Name}, id={episode?.InternalId}");
            var runtime = GetOrCreateAudioFingerprintRuntime();
            if (runtime == null) {
            ...
            var supportedResult = await InvokeMethodAsync(detector, runtime.IsIntroDetectionSupported, supportedArgs)
                .ConfigureAwait(false);
            ...
            await InvokeMethodAsync(detector, runtime.CreateTitleFingerprintAsync, fingerprintArgs)
                .ConfigureAwait(false);

            var season = episode?.Season;
            if (season == null && episode != null) season = libraryManager.GetItemById(episode.ParentId) as Season;
            ...
            var seasonFingerprintInfo =
                await InvokeMethodAsync(detector, runtime.GetAllFingerprintFilesForSeason, getArgs)
                    .ConfigureAwait(false);
            logger.Debug(
                $"GetAllFingerprintFilesForSeason result type: {seasonFingerprintInfo?.GetType().FullName ?? "null"}");

            logger.Debug("触发 UpdateSequencesForSeason 生成片头序列");
            if (seasonFingerprintInfo == null && runtime.SeasonFingerprintInfoType != null &&
                runtime.SeasonFingerprintInfoType.IsClass) {
        ...
        private static async Task<object> InvokeMethodAsync(object target, MethodInfo method, object[] args) {
            ...
            if (result is Task task) {
                ...
                if (taskType.IsGenericType) return taskType.GetProperty("Result")?.GetValue(task);
