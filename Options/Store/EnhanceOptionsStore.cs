    ...
    internal class EnhanceOptionsStore {
        ...
        public void SetOptions(EnhanceOptions options) {
            ...
            if (current.MergeMultiVersion != next.MergeMultiVersion ||
                ...
                MergeMultiVersion.Configure(pluginEnabled && next.MergeMultiVersion, next.MergeSeriesPreference);
            }

            if (next.MergeMultiVersion) Plugin.LibraryService?.EnsureLibraryEnabledAutomaticSeriesGrouping();
        ...
        private static StatusItem BuildChineseSearchTokenizerStatus(EnhanceOptions options) {
            ...
            if (isSimple)
                return new StatusItem("生效中", "当前分词器：simple（中文增强分词）", ItemStatus.Succeeded);

            if (isUnicode61) {
                if (options?.EnhanceChineseSearch == true)
                    return new StatusItem("未生效", "当前分词器：unicode61（默认分词器）", ItemStatus.Warning);

                return new StatusItem("已关闭", "当前分词器：unicode61（默认分词器）", ItemStatus.Succeeded);
            }
