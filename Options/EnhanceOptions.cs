...
namespace MediaInfoKeeper.Options {
    public class EnhanceOptions : EditableOptionsBase {
        ...
        public enum MergeMoviesScopeOption {
            [Description("同文件夹")] FolderScope,
            [Description("同媒体库内")] LibraryScope,
            [Description("所有电影库")] GlobalScope
        }

        public enum MergeSeriesScopeOption {
            [Description("同媒体库内")] LibraryScope,
            [Description("所有电视剧库")] GlobalScope
        }
        ...
        [DisplayName("重建数据库索引")]
        public ButtonItem OptimizeDatabaseButton { get; set; } = new("重建数据库索引") {
            Caption = "重建数据库索引",
        ...
        public StatusItem ChineseSearchTokenizerStatus { get; set; } = new();

        [Browsable(false)] public bool EnhanceChineseSearchRestore { get; set; } = false;

        [Browsable(false)] public bool ShowChineseSearchTokenizerStatus { get; set; } = true;

        [DisplayName("排除原始标题")]
        [Description("从搜索中排除 OriginalTitle 字段")]
        public bool ExcludeOriginalTitleFromSearch { get; set; } = false;

        [Browsable(false)] public List<EditorSelectOption> SearchItemTypeList { get; set; } = new();
        ...
        [Description(
            "关闭时不接管 Emby 原生新入库通知，收藏剧集更新使用插件的 favorites.update 通知；开启时屏蔽 Emby 原生 library.new 通知，并让收藏剧集更新改用 library.new 通知。")]
        ...
        [Browsable(false)] public List<EditorSelectOption> HidePersonOptionList { get; set; } = new();
        ...
        public string SystemLogNameBlacklist { get; set; } =
            "HttpClient;TheMovieDb;SessionsService-;PlaystateService-;MediaInfoService-";
