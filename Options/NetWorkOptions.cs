...
namespace MediaInfoKeeper.Options {
    public class NetWorkOptions : EditableOptionsBase {
        private const string DefaultProxyDomains =
            "mb3admin.com, embydata.com, emby.media, emby.tv, github.com, githubusercontent.com, themoviedb.org, tmdb.org, thetvdb.com, omdbapi.com, fanart.tv, anidb.net, anilist.co, anisearch.de, trakt.tv, opensubtitles.com, musicbrainz.org, theaudiodb.com, discogs.com, telegram.org, discord.com, wikidata.org, mdblist.com";
        ...
        [DisplayName("代理服务器地址")]
        [Description(
            "示例：http://user:pass@127.0.0.1:7890 或 https://127.0.0.1:7890。保存时会通过该代理访问 Google generate_204 进行可用性测试。")]
        ...
        public StatusItem ProxyLatencyStatus { get; set; } = new();
        ...
        [DisplayName("写入环境变量")]
        [Description(
            "同步写入 http_proxy/https_proxy/HTTP_PROXY/HTTPS_PROXY，便于 ffprobe 等外部进程访问需要代理的资源。注意：进程通常无法识别上面的域名列表，开启后可能仍会对所有域名走代理。")]
        ...
        public StatusItem TmdbReplacementStatus { get; set; } = new();

        [Browsable(false)] public bool ShowProxyLatencyStatus { get; set; } = false;

        [Browsable(false)] public bool ShowTmdbReplacementStatus { get; set; } = false;
