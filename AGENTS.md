...
2. Read decompiled source under `~/Documents/Emby/dlls/<emby_version>/source/<AssemblyName>_<version>/` for real
   behavior, access modifiers, inheritance, and usable entry points.
...
- `bash Scripts/decompile` only writes source folders under `~/Documents/Emby/dlls/<emby_version>/source/`; it does not
