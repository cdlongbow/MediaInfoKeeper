...
namespace MediaInfoKeeper.ScheduledTask {
    internal static class RestartReadinessChecker {
        ...
        private static int CountPlaybackUsers(IEnumerable<SessionInfo> sessions) {
            ...
            foreach (var session in sessions) {
                if (session?.HasUser != true || session.NowPlayingItem == null) continue;
    ...
    internal sealed class RestartReadinessStatus {
        ...
        public string Describe() {
            if (PlaybackUserCount > 0 && HasActiveRecording) return $"检测到 {PlaybackUserCount} 个正在播放的用户，且 Live TV 正在录制";

            if (PlaybackUserCount > 0) return $"检测到 {PlaybackUserCount} 个正在播放的用户";

            if (HasActiveRecording) return "检测到 Live TV 正在录制";
