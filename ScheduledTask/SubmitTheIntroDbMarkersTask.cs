...
namespace MediaInfoKeeper.ScheduledTask {
    public class SubmitTheIntroDbMarkersTask : IScheduledTask {
        ...
        public async Task Execute(CancellationToken cancellationToken, IProgress<double> progress) {
            logger.Info("TheIntroDB 上报计划任务开始");

            var apiKey = Plugin.Instance?.Options?.IntroSkip?.TheIntroDbApiKey;
            if (string.IsNullOrWhiteSpace(apiKey)) {
                progress?.Report(100.0);
                logger.Warn("TheIntroDB 上报计划任务跳过: 未配置 API Key");
                return;
            }
            ...
            if (total == 0) {
                progress?.Report(100.0);
                logger.Info("TheIntroDB 上报计划任务完成: 条目数 0");
                return;
            }
            ...
            foreach (var item in items) {
                if (cancellationToken.IsCancellationRequested) {
                    logger.Info("TheIntroDB 上报计划任务已取消");
                    return;
                }
                ...
                try {
                    ...
                    if (result.Succeeded) {
                        ...
                        if (result.RateLimited) {
                            logger.Warn("TheIntroDB 上报计划任务遇到限流，停止本轮任务");
                            break;
                        }
                    }
                }
        ...
        private List<BaseItem> FetchScopedItems() {
            ...
            logger.Info("TheIntroDB 上报候选条目数 {0}, 天数窗口: {1}", items.Count,
