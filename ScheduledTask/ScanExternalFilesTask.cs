...
namespace MediaInfoKeeper.ScheduledTask {
    public class ScanExternalFilesTask : IScheduledTask {
        ...
        public async Task Execute(CancellationToken cancellationToken, IProgress<double> progress) {
            ...
            if (total == 0) {
                progress?.Report(100.0);
                logger.Info("外挂文件扫描计划任务完成，条目数 0");
                return;
            }
            ...
            var tasks = items
                .Select(async item => {
                    try {
                        ...
                        logger.Info($"外挂文件扫描已取消: {item.Path ?? item.Name}");
                    }
                    catch (Exception ex) {
                        logger.Error($"外挂文件扫描失败: {item.Path ?? item.Name}");
        ...
        private List<BaseItem> FetchScopedItems() {
            ...
            logger.Info($"外挂文件扫描条目数 {items.Count}");
            return items;
        }
        ...
        private async Task ProcessItemAsync(
            ...
            if (!Plugin.ExternalFiles.HasExternalFilesChanged(item, refreshOptions.DirectoryService, true)) return;

            await Plugin.ExternalFiles
                .UpdateExternalFiles(item, refreshOptions, false, cancellationToken)
                .ConfigureAwait(false);
            logger.Info($"外挂文件已更新: {displayName}");
        }
    }
}
