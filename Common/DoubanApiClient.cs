    ...
    internal static class DoubanApiClient {
        ...
        public static string BuildSearchUrl(string query) {
            ...
            return url + "?q=" + Uri.EscapeDataString(query) +
                   "&start=0&count=10&apiKey=0dad551ec0f84ed02907ff5c42e8ec70&os_rom=android&_ts=" +
                   Uri.EscapeDataString(ts) + "&_sig=" + Uri.EscapeDataString(sig);
        }
        ...
        public static string BuildCelebritiesUrl(string subjectType, string subjectId) {
            ...
            return endpoint + "?apiKey=0dad551ec0f84ed02907ff5c42e8ec70&os_rom=android&_ts=" +
                   Uri.EscapeDataString(ts) + "&_sig=" + Uri.EscapeDataString(sig);
        }
        ...
        private static string Send(string url, string method, IDictionary<string, string> postData, string acceptHeader) {
            ...
            if (httpClient == null || string.IsNullOrWhiteSpace(url)) return null;
            ...
            try {
                ...
                if (url.IndexOf("api.douban.com", StringComparison.OrdinalIgnoreCase) >= 0) requestOptions.UserAgent = "MediaInfoKeeper";

                using var response = httpClient.SendAsync(requestOptions, method).GetAwaiter().GetResult();
                using var reader = new StreamReader(response.Content);
                var body = reader.ReadToEnd();
                if ((int)response.StatusCode < 200 || (int)response.StatusCode >= 300) {
                    Plugin.SharedLogger?.Debug("DoubanApiClient 请求失败: status={0}, url={1}, body={2}",
