        ...
        public MediaInfoMenuResponse Handle(SetIntroRequest request) {
            ...
            foreach (var episode in targetItems) {
                ...
                try {
                    ...
                    if (creditsStartTicks.HasValue) IntroSkipChapterApi.ReplaceCreditsMarker(chapters, creditsStartTicks.Value);
                    ...
                    response.Succeeded++;
                    Plugin.Instance.Logger.Info(
                        $"ShortcutMenu 设置片头片尾成功: {episode.Path ?? episode.Name}, IntroStart={request.IntroStartTicks}, IntroEnd={request.IntroEndTicks}, CreditsStart={(creditsStartTicks.HasValue ? creditsStartTicks.Value.ToString() : "unchanged")}");
                }
                catch (Exception ex) {
                    response.Failed++;
